...
application development and basic DOM parsing. References:
 + cheerio
...
var checkHtmlFileOrURL = function(htmlfile,checkfile,url) {
    if(url){
        var response2check = buildfn(checkfile);
        restler.get(url).on('complete',response2check);
    } else {
        checkHtmlFile(htmlfile,checkfile)
    }
}
...
var buildfn = function(checkFile) {
    var response2check = function(result, response) {
        if (result instanceof Error) {
            ...
            checkHtml(cheerio.load(result),checkFile);
        }
    };
...
clone(assertFileExists))
        .parse(process.argv);
    var checkJson = checkHtmlFileOrURL(program.file, program.checks,program.url);
} else {
